...
Generates labelled tampered receipt images for training the fraud CNN (Notebook 03).
Supports multiple tamper styles: brightness, duplicate_copy, number_overwrite, handwritten.
...
# ── ADDITIONAL TAMPER MODES (Ranjeet Singh) ───────────────────
...
# ── MAIN BATCH RUNNER ──────────────────────────────────────────
# Iterates over discovered images, applies tamper mode, saves labelled versions
def run(
    input_path: Path,
    output_dir: Path,
    count: int,
