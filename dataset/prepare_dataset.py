...
- **Images present locally: {present}/{len(fraud_rows)}** — the rest live on Google Drive (gitignored).
...
- `synthetic_user_interactions.csv` — {len(inter_rows)} rows across {len({r['user_id'] for r in inter_rows})} users.
