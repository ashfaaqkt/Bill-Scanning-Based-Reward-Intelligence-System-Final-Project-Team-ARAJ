...
### Exactly how synthetic — state this rather than be asked

`dataset/processed/synthetic_user_interactions.csv` holds **772 interactions
across 60 synthetic users**, generated by `build_synthetic_interactions()` in
`dataset/prepare_dataset.py` under a fixed seed, so it regenerates identically.

| Part | Real or invented |
|---|---|
| Merchants and amounts | **Real** — sampled from the CORD and SROIE receipts already in the corpus |
| The user | Invented — `synthetic_user_001` … `_060` |
| The pairing | Invented — each user is assigned one preferred spend category and draws 6–20 receipts with a 60% bias toward it |
| The rating | Invented — 4 in the preferred category, 3 elsewhere, ±1 noise, clipped to 1–5 |

Every row carries `is_synthetic=1`. The consequence is the whole argument for
withholding the metric: **the rating is a deterministic function of a preference
the generator itself assigned, over only three categories.** A matrix
factorisation trained on it can do no better than recover that rule, so a strong
RMSE or NDCG would be evidence about the generator and none at all about the
recommender.

The blocker is the absence of real users — the live Firestore export holds
**19 transactions across three accounts** — not the method.
